...
// src/index.ts
var app = new Hono2();
var TME_BASE = "https://t.me/s/";
var FIRST_SYNC_LIMIT = 5;
function nowSec() {
...
async function tg(env2, method, params, tries = 3) {
  const url = `https://api.telegram.org/bot${env2.BOT_TOKEN}/${method}`;
  ...
  if (data.error_code === 429 && retryAfter && tries > 0) {
    await new Promise((r) => setTimeout(r, retryAfter * 1e3 + 250));
    return tg(env2, method, params, tries - 1);
  }
...
function decodeHtmlEntities(s) {
  ...
  return s.replace(/&([a-zA-Z]+);/g, (_, name) => named[name] ?? `&${name};`).replace(/&#(\d+);/g, (_, d) => {
    const code = Number(d);
...
async function fetchTme(username) {
...
__name(fetchTme, "fetchTme");
function scrapeTmePreview(username, html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    const start = m.index;
    const slice = html.slice(start, start + 5e4);
    const textMatch = /<div class="tgme_widget_message_text[^"]*">([\s\S]*?)<\/div>/.exec(slice) || /<div class="tgme_widget_message_text[^>]*>([\s\S]*?)<\/div>/.exec(slice);
    const raw2 = textMatch ? textMatch[1] : "";
    const text = raw2 ? stripHtml(raw2) : "";
    posts.push({
      postId,
      text,
...
__name(scrapeTmePreview, "scrapeTmePreview");
async function sendPostToDestination(env2, destChatId, username, post) {
...
${body}`.slice(0, 3900);
  await tg(env2, "sendMessage", { chat_id: destChatId, text: msg });
}
__name(sendPostToDestination, "sendPostToDestination");
async function handlePrivateMessage(env2, msg) {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const text = msg.text || "";
  await upsertUser(env2.DB, userId);
  const send2 = /* @__PURE__ */ __name((t) => tg(env2, "sendMessage", { chat_id: chatId, text: t }), "send");
  ...
  if (parsed.cmd === "/follow") {
    ...
    if (!u) return send2("Usage: /follow @channelusername");
    await env2.DB.prepare(
    ...
    ).bind(userId, u, nowSec()).run();
    return send2(`\u2705 Following @${u}. I\u2019ll check it on the schedule.`);
  }
  if (parsed.cmd === "/unfollow") {
    const u = normalizeUsername(parsed.args[0] || "");
    if (!u) return send2("Usage: /unfollow @channelusername");
    await env2.DB.prepare("DELETE FROM user_sources WHERE user_id=? AND username=?").bind(userId, u).run();
    return send2(`\u{1F9F9} Unfollowed @${u}.`);
  }
  if (parsed.cmd === "/list") {
    const dest = await env2.DB.prepare("SELECT chat_id, verified FROM destinations WHERE user_id=?").bind(userId).first();
    const follows = await env2.DB.prepare(
    ...
    return send2(
      [
        ...
        `Following: ${follows.results.length ? follows.results.map((r) => `@${r.username}`).join(", ") : "none"}`
      ].join("\n")
    );
  }
  return send2("Unknown command. Try /start");
}
__name(handlePrivateMessage, "handlePrivateMessage");
function parseDestClaim(text) {
  const t = (text || "").trim();
  const m = /^DEST\s+([A-Za-z0-9_-]{6,64})$/.exec(t);
  return m ? m[1] : null;
}
__name(parseDestClaim, "parseDestClaim");
async function handleChannelPost(env2, msg) {
  const chatId = msg.chat.id;
  ...
  await env2.DB.prepare("DELETE FROM pending_claims WHERE token=?").bind(token).run();
  await tg(env2, "sendMessage", { chat_id: userId, text: `\u2705 Destination verified: ${chatId}` });
}
__name(handleChannelPost, "handleChannelPost");
async function processUpdate(env2, update) {
  if (update.message && update.message.chat?.type === "private") {
    await handlePrivateMessage(env2, update.message);
    return;
  }
  if (update.channel_post) {
    await handleChannelPost(env2, update.channel_post);
    return;
  }
}
__name(processUpdate, "processUpdate");
async function runScrapeTick(env2) {
  const followed = await env2.DB.prepare("SELECT DISTINCT username FROM user_sources").all();
  for (const row of followed.results) {
    const username = String(row.username);
    try {
      const source = await env2.DB.prepare("SELECT last_post_id FROM sources WHERE username=?").bind(username).first();
      const lastSeen = Number(source?.last_post_id ?? 0);
      const html = await fetchTme(username);
      const posts = scrapeTmePreview(username, html);
      if (!posts.length) {
        await env2.DB.prepare("UPDATE sources SET updated_at=? WHERE username=?").bind(nowSec(), username).run();
        continue;
      }
      ...
      const subs = await env2.DB.prepare(
      ...
      for (const post of newPosts) {
        for (const sub of subs.results) {
          ...
          ).bind(userId, username, post.postId).first();
          if (exists) continue;
          try {
            await sendPostToDestination(env2, destChatId, username, post);
            await env2.DB.prepare(
              "INSERT INTO deliveries(user_id, username, post_id, created_at) VALUES(?, ?, ?, ?)"
            ).bind(userId, username, post.postId, nowSec()).run();
          } catch (e) {
      ...
      await env2.DB.prepare("UPDATE sources SET last_post_id=?, updated_at=? WHERE username=?").bind(maxId, nowSec(), username).run();
    } catch (e) {
...
__name(runScrapeTick, "runScrapeTick");
app.get("/", (c) => c.text("ok"));
...
var src_default = {
  fetch: app.fetch,
  scheduled: /* @__PURE__ */ __name(async (_controller, env2, ctx) => {
    ctx.waitUntil(runScrapeTick(env2));
  }, "scheduled")
};
...
// .wrangler/tmp/bundle-HATamr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HATamr/middleware-loader.entry.ts
